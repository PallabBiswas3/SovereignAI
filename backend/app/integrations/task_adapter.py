...
def integrated_result_state(
    *, request: str, routing: Any, selection: Any, plan: IntegrationRoutePlan,
    ...
    steps.append(AgentStep(
        id=len(steps) + 1, action="local_model_synthesis", title="Generate an evidence-bounded candidate",
        status=(
            StepStatus.completed
            if result.service_status.get("model") in {"available", "not_requested"}
            else StepStatus.failed
        ),
        observation=(
            f"Candidate generated by {(result.model_runtime or {}).get('provider', 'caller-supplied response')}."
            if result.service_status.get("model") != "unavailable"
            else "The configured local model was unavailable; no candidate was fabricated."
        ),
        verification="Only evidence supplied by the authorized retrieval and diagnostic stages entered the prompt.",
    ))
    steps.append(AgentStep(
...
def _graph_source(chunk: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "authorization": chunk.get("authorization") or {
            "decision": "allowed" if chunk.get("authorization_scope") else "not_reported",
            "scope_fingerprint": chunk.get("authorization_scope"),
        },
    }
