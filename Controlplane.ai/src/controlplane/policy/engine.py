...
class PolicyEngine:
    ...
    def decide(
        ...
        *,
        ...
        risk_score = max(
            (_SEVERITY_SCORE[item.severity] * item.confidence for item in findings),
            default=0.0,
        )
        depth = self._verification_depth(profile)
        reason = (
            f"Applied {len(set(matched))} policy rule(s); strongest action is {action.value}."
            if matched
            else f"No policy rule matched; applied default action {action.value}."
        )
        return PolicyDecision(
            action=action,
            verification_depth=depth,
            risk_score=min(1.0, float(risk_score)),
            reason=reason,
            matched_rules=list(dict.fromkeys(matched)),
            human_review_required=action == EnforcementAction.REVIEW,
    ...
    def _matches(rule: PolicyRule, finding: Finding, interaction: Interaction) -> bool:
        ...
        if condition.status is not None and finding.status != condition.status:
            return False
        if condition.severity_at_least is not None:
            if SEVERITY_RANK[finding.severity] < SEVERITY_RANK[condition.severity_at_least]:
                return False
